...
static const uint8_t  SCAN_SECS = 3;
static unsigned long  sPreAnimStart = 0;
static const unsigned long PRE_ANIM_MS = 700;
...
static void termTick(TFT_eSPI& tft) {
  ...
  String msg;
  switch ((sTermPhase++) % 4) {
    case 0: msg = "["+String(spc)+"] root@infiltra:/# hciconfig hci0 up"; break;
    case 1: msg = "["+String(spc)+"] root@infiltra:/# hcitool lescan --ch " + chStr; break;
    case 2: msg = "["+String(spc)+"] root@infiltra:/# sniff --adv --short --timeout=150ms"; break;
...
static void drawSignalBars(TFT_eSPI& tft, int centerX, int baselineY, int rssi, uint16_t col=TFT_WHITE) {
  ...
  for (int i=0;i<4;i++) {
    int bh = 3 + i*3;
    int bx_ = x0 + i*(w+gap);
    int by_ = baselineY - bh;
    tft.drawRect(bx_, by_, w, bh, col);
    if (i < bars) tft.fillRect(bx_+1, by_+1, w-2, bh-2, col);
  }
}
...
static void doScanBlocking() {
  ...
  if (!sScan) {
    ...
    sScan->setInterval(120);
    sScan->setWindow(80);
  }

  BLEScanResults res = sScan->start(SCAN_SECS, false);
  int n = res.getCount();
  for (int i=0; i<n && sCount<64; ++i) {
    BLEAdvertisedDevice d = res.getDevice(i);
    DevInfo out;
    if (d.haveName()) out.name = String(d.getName().c_str());
    if (!out.name.length()) out.name = "<unknown>";
    out.addr = String(d.getAddress().toString().c_str());
    out.rssi = d.getRSSI();
    if (d.haveTXPower()) { out.hasTx = true; out.tx = d.getTXPower(); }
    if (d.haveServiceUUID()) {
      out.hasUUID = true;
      out.uuid = String(d.getServiceUUID().toString().c_str());
    }

    if (d.haveManufacturerData()) {
      std::string mfg = d.getManufacturerData();
      out.mfgLen = (int)mfg.size();
    }
...
static void startAsyncScan() {
  ...
  sTermPhase = 0;
  sLastTermMs = millis();
  sPreAnimStart = millis();
  sScanCompleteFlash = false;
  sScanCompleteShown = false;
  sScanCompleteMs    = 0;
}

void bleScanReset() { startAsyncScan(); }
...
void bleScanDrawScreen(TFT_eSPI& tft) {
  ...
  if (sScanCompleteFlash) {
    ...
    if (millis() - sScanCompleteMs < 650) return;
    sScanCompleteFlash = false;
    sDirty = true;
  }
  ...
  if (sBusy) {
    ...
    if (now - sPreAnimStart >= PRE_ANIM_MS) {
      doScanBlocking();
  ...
  if (sDirty) {
    ...
    const DevInfo& d = sList[sSel];
    int y = cy;

    {
      String nm = d.name.length() ? d.name : String("<unknown>");
      String left = "NAME: " + clipToWidth(tft, nm, cw - 60);
    ...
    {
      String line = "ADDR: " + clipToWidth(tft, d.addr, cw - tft.textWidth("ADDR: "));
      tft.setCursor(cx, y); tft.print(line);
      y += 14;
    }

    {
      drawBadge(tft, cx, y, "ADV BLE", TFT_DARKGREY, 0x0000, TFT_WHITE);
      String rightText;
      if (d.hasUUID && d.uuid.length()) {
        String u = d.uuid;
        if (u.length() > 10) u = u.substring(0, 10) + "...";
        rightText = "UUID " + u;
      } else {
        rightText = "MFG " + String(d.mfgLen) + "B";
      }
      int leftW = tft.textWidth("ADV BLE") + 8;
      int x2 = cx + leftW + 10;
      drawBadge(tft, x2, y, rightText, TFT_DARKGREY, 0x0000, TFT_WHITE);

      y += 20;
    }

    {
      String rssiTxt = "RSSI: " + String(d.rssi) + " dBm";
...

    {
      ...
      drawSignalBars(tft, bx + bw/2, barsBaseline, d.rssi);
    }

    tft.setTextColor(TFT_SILVER, TFT_BLACK);
    tft.setCursor(cx, by + bh - 34);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
